...
rpydot 	= mp*cos(phi+pi)					;	% d(rpy)/dt (using problem 2)
rsxdot 	= -mp*sin(phi)						;	% d(rsx)/dt (using problem 2)
rsydot 	= ms*cos(phi)						;	% d(rsy)/dt (using problem 2)

t 		= (sqrt(2)/3) * (D^(3/2)) * ...
		  (tan(f0/2))*(3+(tan(f0/2))^2)		;	% initial time t0
x(1) 	= (R0*cos(f0)) + (mp*cos(phi+pi))	;	% x_p
x(2) 	= (R0*sin(f0)) + (mp*sin(phi+pi))	;	% y_p
x(3) 	= xcmxdot+rpxdot					;	% v_xp
x(4) 	= xcmydot+rpydot					;	% v_yp
x(5) 	= (R0*cos(f0)) + (ms*cos(phi))		;	% x_s
x(6) 	= (R0*sin(f0)) + (ms*sin(phi))		;	% y_s
